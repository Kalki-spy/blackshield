    ...
    def do_GET(self):
        ...
        if parsed.path == "/find":
            ...
            if not domain:
                ...
                return
            scan_id = activity_log.start_scan(user_id, "Subdomain Finder", domain) if user_id else None
            try:
                result = find_subdomains(domain)
                if user_id and not result.get("error"):
                    if result.get("total", 0) > 0:
                        activity_log.add_finding(
                            user_id, "Subdomain Finder", domain,
                            f"{result['total']} subdomains discovered, expanding attack surface",
                            severity="info", scan_id=scan_id,
                        )
                    activity_log.finish_scan(scan_id, status="completed", tool="Subdomain Finder", target=domain, user_id=user_id)
                self._json(200, result)
            except Exception as e:
                if user_id and scan_id:
                    activity_log.finish_scan(scan_id, status="failed", tool="Subdomain Finder", target=domain, user_id=user_id)
