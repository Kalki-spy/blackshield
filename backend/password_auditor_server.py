    ...
    def do_POST(self):
        ...
        if path == "/password/analyze":
            ...
            if user_id and not result.get("error"):
                scan_id = activity_log.start_scan(user_id, "Password Auditor", "password sample")
                if result.get("score_pct", 100) < 45:
                    activity_log.add_finding(
                        user_id, "Password Auditor", "password sample",
                        f"Weak password identified ({result.get('strength', 'weak')})",
                        severity="medium", scan_id=scan_id,
                    )
                activity_log.finish_scan(scan_id, status="completed", tool="Password Auditor", target="password sample", user_id=user_id)
            _json(self,200,result)
        elif path == "/password/bulk":
            pwds = body.get("passwords",[])
            user_id = body.get("user_id")
            results = [{"password":p, **analyze_password(p)} for p in pwds[:50]]
            weak_count = sum(1 for r in results if r.get("score_pct",0) < 45)
            if user_id:
                scan_id = activity_log.start_scan(user_id, "Password Auditor", f"{len(results)} passwords")
                if weak_count:
                    activity_log.add_finding(
                        user_id, "Password Auditor", f"{len(results)} passwords",
                        f"{weak_count} of {len(results)} passwords audited are weak",
                        severity="medium" if weak_count < len(results) else "high", scan_id=scan_id,
                    )
                activity_log.finish_scan(scan_id, status="completed", tool="Password Auditor", target=f"{len(results)} passwords", user_id=user_id)
