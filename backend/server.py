...
class SSLHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/analyze":
            ...
            if not url:
                self._json(400, {"error": "Missing required query parameter: url"})
                return
            scan_id = activity_log.start_scan(user_id, "SSL Analyzer", url) if user_id else None
            try:
                result = analyze_ssl(url)
                if user_id and not result.get("error"):
                    for r in result.get("results", []):
                        if r.get("status") != "secure":
                            activity_log.add_finding(
                                user_id, "SSL Analyzer", url,
                                f"{r.get('item')}: {r.get('description')}",
                                severity=r.get("severity", "medium"), scan_id=scan_id,
                            )
                    activity_log.finish_scan(scan_id, status="completed", tool="SSL Analyzer", target=url, user_id=user_id)
                self._json(200, result)
            except Exception as e:
                if user_id and scan_id:
                    activity_log.finish_scan(scan_id, status="failed", tool="SSL Analyzer", target=url, user_id=user_id)
...
if __name__ == "__main__":
    server = HTTPServer(("0.0.0.0", PORT), SSLHandler)
    print(f"[SSL Analyzer] Running on:{PORT}")
    print(f"  GET /health          — health check")
    print(f"  GET /analyze?url=... — analyze SSL/TLS")
