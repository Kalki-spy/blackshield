...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        qs = parse_qs(p.query)
        if p.path == "/health":
            self._json(200, {"status": "ok", "port": PORT}); return
        if p.path == "/scan":
            target    = qs.get("target",    [""])[0].strip()
            mode      = qs.get("mode",      ["both"])[0].lower()
            intensity = max(1, min(int(qs.get("intensity", ["2"])[0]), 3))
            user_id   = qs.get("user_id", [None])[0]
            user_id   = int(user_id) if user_id else None
            if not target:
                self._json(400, {"error": "Missing parameter: target"}); return
            if mode not in ("remote","client","both"): mode = "both"
            scan_id = activity_log.start_scan(user_id, "Sniper", target) if user_id else None
            try:
                result = run_sniper(target, mode=mode, intensity=intensity)
                if user_id and not result.get("error"):
                    for cve in result.get("cves", []):
                        activity_log.add_finding(
                            user_id, "Sniper", target,
                            f"{cve.get('cve')} — {cve.get('name')}",
                            severity=cve.get("severity", "medium"), scan_id=scan_id,
                        )
                    activity_log.finish_scan(scan_id, status="completed", tool="Sniper", target=target, user_id=user_id)
                self._json(200, result)
            except Exception as e:
                if user_id and scan_id:
                    activity_log.finish_scan(scan_id, status="failed", tool="Sniper", target=target, user_id=user_id)
...
if __name__ == "__main__":
    ...
    print(f"[Sniper] Running on:{PORT}")
    print(f"  GET /scan?target=https://example.com&mode=both&intensity=2")
    srv.serve_forever()