...
import activity_log
"""
Metasploit-style Vulnerability Scanner Backend
Pure Python — no external dependencies

Endpoints:
    GET  /health              — health check
    POST /scan                — scan a target
        body: { target, ports?, scan_type? }
"""
...
class MetasploitHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/scan":
            ...
            if not target:
                self.json_resp(400, {"error": "target is required"})
                return
            scan_id = activity_log.start_scan(user_id, "Metasploit", target) if user_id else None
            try:
                result = scan_target(target, ports, scan_type)
                if user_id:
                    for cve in result.get("cves", []):
                        activity_log.add_finding(
                            user_id, "Metasploit", target,
                            f"{cve.get('id')} — {cve.get('name')} (port {cve.get('port')})",
                            severity=cve.get("severity", "medium"), scan_id=scan_id,
                        )
                    activity_log.finish_scan(scan_id, status="completed", tool="Metasploit", target=target, user_id=user_id)
                self.json_resp(200, result)
            except Exception as e:
                if user_id and scan_id:
                    activity_log.finish_scan(scan_id, status="failed", tool="Metasploit", target=target, user_id=user_id)
...
if __name__ == "__main__":
    ...
    server = HTTPServer(("0.0.0.0", PORT), MetasploitHandler)
    print(f"[Metasploit] Running on:{PORT}")
    print(f"  GET  /health  — health check")
    print(f"  POST /scan    — vulnerability scan")
