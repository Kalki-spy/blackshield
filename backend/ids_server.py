...
class IDSHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            self.send_json(200, {"status": "ok", "port": PORT, "vt_enabled": bool(VT_API_KEY)})
        else:
    ...
    def do_POST(self):
        ...
        scan_type = body.get("scan_type", "url")
        ...
        try:
            if VT_API_KEY and scan_type == "url":
                result = vt_scan_url(target)
            else:
                result = simulate_scan(target, scan_type)
...
if __name__ == "__main__":
    try:
        ...
        print(f"[IDS] Server running on port {PORT} | VT API: {'enabled' if VT_API_KEY else 'disabled (simulation mode)'}")
        server.serve_forever()
    except OSError as e:
        print(f"[IDS] Failed to start on port {PORT}: {e}")