...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path == "/scan":
            ...
            param_str  = qs.get("params", [""])[0].strip()
            level      = max(1, min(int(qs.get("level", ["2"])[0]), 3))
            ...
            scan_id = activity_log.start_scan(user_id, "SQL Injection Scanner", url) if user_id else None
            try:
                result = run_scan(url, param_names=param_names, level=level)
                if user_id and not result.get("error"):
                    for f in result.get("findings", []):
                        activity_log.add_finding(
                            user_id, "SQL Injection Scanner", url,
                            f"{f.get('injection_type', 'SQL injection')} in parameter '{f.get('parameter')}'",
                            severity=f.get("severity", "medium"), scan_id=scan_id,
                        )
                    activity_log.finish_scan(scan_id, status="completed", tool="SQL Injection Scanner", target=url, user_id=user_id)
                self._json(200, result)
            except Exception as e:
                if user_id and scan_id:
                    activity_log.finish_scan(scan_id, status="failed", tool="SQL Injection Scanner", target=url, user_id=user_id)
...
if __name__ == "__main__":
    ...
    print(f"[SQLMap] Running on:{PORT}")
    print(f"  GET /health")
    print(f"  GET /scan?url=https://target.com/page?id=1&level=2")
    httpd.serve_forever()