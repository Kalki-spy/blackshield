...
class HashcatHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/crack":
            ...
            target   = hash_str[:16] + "…" if len(hash_str) > 16 else hash_str
            scan_id  = activity_log.start_scan(user_id, "Hashcat", target) if user_id else None
            try:
                result = crack_hash(
                    hash_str    = hash_str,
                ...
                if user_id and not result.get("error"):
                    if result.get("cracked"):
                        activity_log.add_finding(
                            user_id, "Hashcat", target,
                            "Weak password hash cracked — password reuse/policy risk",
                            severity="high", scan_id=scan_id,
                        )
                    activity_log.finish_scan(scan_id, status="completed", tool="Hashcat", target=target, user_id=user_id)
                self.json_resp(200, result)
            except Exception as e:
                if user_id and scan_id:
                    activity_log.finish_scan(scan_id, status="failed", tool="Hashcat", target=target, user_id=user_id)
